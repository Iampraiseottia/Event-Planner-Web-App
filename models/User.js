...
class User {
  ...
  static async findById(id) {
    ...
    SELECT u.*, p.business_name, p.bio, p.experience, p.specializations, 
           ...
           u.profile_image_data, u.profile_image_mime_type -- Added image data and mime type
    FROM users u
  ...
  `;
    ...
    if (user.specializations && typeof user.specializations === "string") {
      try {
        ...
        console.error("Error parsing specializations for user:", id, e);
        user.specializations = [];
      }
    } else if (!user.specializations) {
      user.specializations = [];
    }
  ...
  `;
    ...
    if (user.specializations && typeof user.specializations === "string") {
      try {
        ...
        console.error("Error parsing specializations for user:", email, e);
        user.specializations = [];
      }
    } else if (!user.specializations) {
      user.specializations = [];
  ...
  static async update(id, userData) {
    ...
    RETURNING id, full_name, email, phone_number, user_type, location, 
              date_of_birth, preferences, created_at, updated_at,
              profile_image_data, profile_image_mime_type -- Return new image fields
  `;
  ...
  static async updatePlannerProfile(userId, plannerData) {
    ...
    const checkQuery = `SELECT id FROM planners WHERE user_id = $1`;
    const existingPlanner = await pool.query(checkQuery, [userId]);

    let query, values;

    if (existingPlanner.rows.length === 0) {
      // Insert new planner record
      query = `
  ...
  `;
    ...
    if (user.specializations && typeof user.specializations === "string") {
      try {
        ...
        console.error("Error parsing specializations for user:", userId, e);
        user.specializations = [];
      }
    } else if (!user.specializations) {
      user.specializations = [];
